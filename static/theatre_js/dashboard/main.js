...
function appendYearlyData(data) {
    const month = data.month;
    const revenue = data.revenue;
    yearly_data[month] += revenue
    createRevenueChart(yearly_data);
}
...
function ShowRevenueChart(year) {
    ...
    // clear teh yearly data..
    clearYearlyData()

    eventSource.onmessage = (e) => {
        try {
            const data = JSON.parse(e.data)
            const month = data.payment_time.split("-")[1]
            revenue_data = {'month': month, 'revenue': data.order_amount}
            appendYearlyData(revenue_data)
        }
        catch (error) {
            console.log(error)
        }
